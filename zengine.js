...
class Camera { constructor(C=new V,R = new V) { this.C = C; this.R = R; }}
...
function rotate(v,r) { const [x,y,z] = [r.x*dg, r.y*dg, r.z*dg], [cx,cy,cz,sx,sy,sz] = [c(x), c(y), c(z), s(x), s(y), s(z)];
    ...
    /*Demo const*/const rZ = (vysx * cy) + (-v.x*sy) + (vzcx * cy); /*Rotation of the Z axi*/ const rW = v.w;
    return new V(pF(rX), pF(rY), pF(rZ), pF(rW)); }
...
function transforms(obj) {
    ...
    for(let i=0; i < b.length; i++) { b[i] = rotate(translate(rotate(scale(b[i],obj.S),obj.R),obj.C),cam.R); }
    return b;
}
...
var objs = [], toDraw = [];
var cam = new Camera();
var origin = new Mesh("origin_axis.obj", new V(0,0,+50));
